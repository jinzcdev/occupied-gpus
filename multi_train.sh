...
# for any pytorch version
#CUDA_DEVICE_ORDER="PCI_BUS_ID" \
#OMP_NUM_THREADS=1 \
#CUDA_VISIBLE_DEVICES=$gpu_ids \
#python -m torch.distributed.launch --use_env --nproc_per_node=$num_gpus --master_port=$port  ./occupiedgpus/multi_core.py \
#--batch-size 8 --lr 1e-4 --weights ./runs/checkpoint.pth --eval-interval 3 --save-root ./runs \
#--gpu-ids $gpu_ids  --option $option